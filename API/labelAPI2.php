...
switch($fn){
	case "getdata" :
				$DB = new exDB;
				$DB->GetData("SELECT `lbDegree`, `suName`, `FactoryID`, `lbLicense`, `lbIssueDate`, `lbExpireDate`, `faLat`, `faLong`, `lbBrand`, `lbContact`, `lbFacName`, `lbPicture`, `lbAddress` FROM `Label`, `Factory`,`SuraType` WHERE `FactoryID` = `lbFacCode` AND SuraTypeID = lbType AND lbBrand = ? ORDER BY FactoryID",array("s",$_GET["label"]));
	...
	case "autocomplete" :
				$data = null;
				if(strlen($_GET["label"]) > 1){ $DB = new exDB;
					$DB->GetData("SELECT `LabelID`, `lbBrand` FROM `Label` WHERE lbBrand LIKE ?",array("s","%".$_GET["label"]."%"));
					if($DB->GetNumRows() > 0){
						$data = array();
